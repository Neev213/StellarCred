#![no_std]
//! CredentialVerifier
//!
//! Stateless cryptographic gateway. One `verify_*` function per credential type;
//! each looks up the verification key for its circuit and checks an UltraHonk
//! proof against the supplied public inputs. It stores no per-holder state and
//! returns only a boolean.
//!
//! The verification keys are set by an admin (one VK per credential circuit),
//! mirroring the `rs-soroban-ultrahonk` pattern where the VK is fixed at deploy
//! time. `proof` and `public_inputs` are opaque byte blobs in the format
//! emitted by Barretenberg (`bb`).
//!
//! NOTE (skeleton): real verification is stubbed in `ultrahonk::verify`. It
//! currently performs a structural sanity check only. Wiring the actual
//! UltraHonk verifier (rs-soroban-ultrahonk) is the next step — see README.

use soroban_sdk::{
    contract, contracterror, contractimpl, contracttype, panic_with_error, symbol_short, Address,
    Bytes, Env, Symbol,
};

#[contracttype]
pub enum DataKey {
    Admin,
    /// Verification key bytes, keyed by credential-type symbol.
    Vk(Symbol),
}

#[contracterror]
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
#[repr(u32)]
pub enum Error {
    NotInitialized = 1,
    VkNotSet = 2,
}

#[contract]
pub struct CredentialVerifier;

#[contractimpl]
impl CredentialVerifier {
    pub fn __constructor(env: Env, admin: Address) {
        env.storage().instance().set(&DataKey::Admin, &admin);
    }

    /// Register/replace the verification key for a credential circuit. Admin-only.
    pub fn set_vk(env: Env, credential_type: Symbol, vk: Bytes) {
        Self::require_admin(&env);
        env.storage()
            .persistent()
            .set(&DataKey::Vk(credential_type), &vk);
    }

    pub fn verify_kyc_proof(env: Env, proof: Bytes, public_inputs: Bytes) -> bool {
        Self::verify(&env, symbol_short!("kyc"), &proof, &public_inputs)
    }

    pub fn verify_age_proof(env: Env, proof: Bytes, public_inputs: Bytes) -> bool {
        Self::verify(&env, symbol_short!("age"), &proof, &public_inputs)
    }

    pub fn verify_jurisdiction_proof(env: Env, proof: Bytes, public_inputs: Bytes) -> bool {
        Self::verify(&env, Symbol::new(&env, "jurisdiction"), &proof, &public_inputs)
    }

    pub fn verify_income_proof(env: Env, proof: Bytes, public_inputs: Bytes) -> bool {
        Self::verify(&env, symbol_short!("income"), &proof, &public_inputs)
    }

    fn verify(env: &Env, credential_type: Symbol, proof: &Bytes, public_inputs: &Bytes) -> bool {
        let vk: Bytes = env
            .storage()
            .persistent()
            .get(&DataKey::Vk(credential_type))
            .unwrap_or_else(|| panic_with_error!(env, Error::VkNotSet));
        ultrahonk::verify(env, &vk, proof, public_inputs)
    }

    fn require_admin(env: &Env) {
        let admin: Address = env
            .storage()
            .instance()
            .get(&DataKey::Admin)
            .unwrap_or_else(|| panic_with_error!(env, Error::NotInitialized));
        admin.require_auth();
    }
}

mod ultrahonk {
    use soroban_sdk::{Bytes, Env};

    /// STUB. Replace with the real UltraHonk verifier from rs-soroban-ultrahonk.
    /// Returns true only if the inputs are structurally present. This lets the
    /// end-to-end flow (issue → prove → submit → gate) run before the heavy
    /// cryptography is wired in.
    pub fn verify(_env: &Env, vk: &Bytes, proof: &Bytes, public_inputs: &Bytes) -> bool {
        !vk.is_empty() && !proof.is_empty() && !public_inputs.is_empty()
    }
}

mod test;
